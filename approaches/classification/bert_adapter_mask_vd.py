...
class Appr(ApprBase):
    ...
    def train(self,t,train,valid,num_train_steps,train_data,valid_data):
        ...
        datasize = len(train)
        if self.args.KL_coeff == '1':
            self.KL_coeff = 1
        elif self.args.KL_coeff == '1_M':
            self.KL_coeff = 1/self.train_batch_size
        elif self.args.KL_coeff == '1_N':
            self.KL_coeff = 1/datasize
        elif self.args.KL_coeff == 'M_N':
            self.KL_coeff = self.train_batch_size/datasize
    ...
    def train_epoch(self,t,data,iter_bar,optimizer,t_total,global_step,e):
        ...
        for step, batch in enumerate(iter_bar):
            ...
            # supervised CE loss ===============
            output_dict,kld = self.model(t,input_ids, segment_ids, input_mask,s=s)
            ...
            # print("CE loss:", self.ce(output,targets))
            # print("HAT loss:", self.hat_reg(output,targets,masks))
            loss = self.ce(output,targets) + self.hat_reg(output,targets,masks) #output_ce is the output of head (no softmax)

            # KL loss
            loss += kld * self.KL_coeff * self.args.KL_weight
            ...
            if self.args.sup_loss:
                ...
                # print("sup_loss:", self.sup_loss(output,pooled_rep,input_ids, segment_ids, input_mask,targets,t,s))
            ...
            for step, batch in enumerate(data):
                ...
                if 'dil' in self.args.scenario:
                    ...
                    if self.args.last_id: # use the last one
                        output_dict,_ = self.model(trained_task,input_ids, segment_ids, input_mask,s=self.smax, noise=self.noise[t])
                        output = output_dict['y']
                        masks = output_dict['masks']
                        ...
                        for e in entrop_to_test:
                            output_dict,_ = self.model(e,input_ids, segment_ids, input_mask,s=self.smax, noise=self.noise[t])
                            output = output_dict['y']
                            masks = output_dict['masks']
                    ...
                    output_dict,_=self.model.forward(task,input_ids, segment_ids, input_mask,s=self.smax, noise=self.noise[t])
                    outputs = output_dict['y']
                    masks = output_dict['masks']
                    output = outputs[t]


                loss=self.ce(output,targets) + self.hat_reg(output,targets,masks)
