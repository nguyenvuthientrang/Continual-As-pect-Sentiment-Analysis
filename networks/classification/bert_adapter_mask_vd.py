...
from dropout import GaussDropout
...
class Net(torch.nn.Module):
    ...
    def forward(self,t,input_ids, segment_ids, input_mask, start_mixup=None,s=None,noise=None):
        output_dict = {}
        kld = []
        ...
        if start_mixup:
            ...
            if self.training:
              pooled_output,kl = self.dropout(pooled_output)
              kld.append(kl)
            else:
              pooled_output,kl = self.dropout(pooled_output, noise['dropout.log_alpha'])
              kld.append(kl)
            ...
            sequence_output, pooled_output = \
                self.bert(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask,t=t,s=s)
            masks = self.mask(t,s)
            # print("Pooled Output after Bert:", pooled_output.size())
            if self.training:
              pooled_output,kl = self.dropout(pooled_output)
              kld.append(kl)
            else:
              pooled_output,kl = self.dropout(pooled_output, noise['dropout.log_alpha'])
              kld.append(kl)
            # print("Pooled Output after Dropout:", pooled_output.size())
